...
public class UniqueEmail {
    ...
    public static int numUniqueEmails(String[] emails) { // O(n * m) = 24ms
        Set<String> uniqueEmails = new HashSet<>();
        for (int i = 0; i < emails.length; i++) {
            String[] email = emails[i].split("@");
            String localName = email[0].split("[+]")[0].replace(".", "");
            uniqueEmails.add(localName + "@"+ email[1]);
        }
        System.out.println(uniqueEmails);
        return uniqueEmails.size();
    }

    public static int numUniqueEmails2(String[] emails) { // 19ms
        Set<String> uniqueEmails = new HashSet<>();
        for (int i = 0; i < emails.length; i++) {
            String[] email = emails[i].split("@");
            String localName = email[0].replace(".", "");
    ...
    public static int numUniqueEmails3(String[] emails) { // 9ms
        Set<String> uniqueEmails = new HashSet<>();
